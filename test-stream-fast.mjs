...
async function run() {
    try {
        ...
        const outStream = fs.createWriteStream('test-streamed-fast.mp3');

        while (true) {
            const { done, value } = await reader.read();
            if (done) {
                if (remainder.length > 0) {
                    outStream.write(remainder);
                }
                outStream.end();
                break;
            }
        ...
        console.log("Streaming done in " + (Date.now() - start) + "ms. Saved as test-streamed-fast.mp3");
